...
export default function Navbar() {
    ...
    return (
        <header className={`${isAuthPage ? "bg-[#fbfbfb]" : "bg-background"} sticky top-0 z-50 border-b border-border shadow-sm transition-transform duration-300 ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
            <nav className="flex h-16 w-full items-center justify-between gap-4 pl-4 pr-2 sm:pr-4">
                ...
                <div className="flex flex-1 items-center gap-2 min-w-0">
                    <MenubarToggle />
                    <Link href="/" className="shrink-0 flex items-center" aria-label="LandGo Home">
                    ...
                    <div className="hidden flex-1 items-center gap-2 md:flex max-w-3xl px-2">
                        <div className="relative flex-1">
                            ...
                            <Input className="pl-9 h-10 w-full bg-slate-50 border-none shadow-none focus-visible:ring-1 focus-visible:ring-slate-300" placeholder="Tìm kiếm bất động sản" aria-label="Search estates" />
                        </div>
                        <FilterDropdown />
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0 pr-2">
                    ...
                    <Button asChild className="inline-flex shadow-sm text-xs sm:text-sm px-4 h-9">
            ...
            <div className="flex px-4 pb-3 gap-2 md:hidden">
                <div className="relative flex-1">
                    <Search className="text-muted-foreground pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2" />
                    <Input className="pl-9 h-10" placeholder="Tìm kiếm bất động sản" aria-label="Search estates mobile" />
                </div>
                <FilterDropdown />
            </div>
        </header>
